...
#Read Data
trans_distA = meshio.read(transmural_pathA)
trans_distB = meshio.read(transmural_pathB)
...
transA_endo = np.zeros((meshVtk.points.shape[0]))
transA_endo[trans_distA.point_data['f_20']<phi_endo] = 1
transA_endo = transA_endo.astype(bool)

transA_epi = np.zeros((meshVtk.points.shape[0]))
transA_epi[trans_distA.point_data['f_20']>phi_epi] = 1
transA_epi = transA_epi.astype(bool)

transA_mid = np.zeros((meshVtk.points.shape[0]))
transA_mid[(trans_distA.point_data['f_20']<=phi_epi) & (trans_distA.point_data['f_20']>=phi_endo)] = 1
transA_mid = transA_mid.astype(bool)

transB_endo = np.zeros((meshVtk.points.shape[0]))
transB_endo[trans_distB.point_data['f_20']<phi_endo] = 1
transB_endo = transB_endo.astype(bool)

transB_epi = np.zeros((meshVtk.points.shape[0]))
transB_epi[trans_distB.point_data['f_20']>phi_epi] = 1
transB_epi = transB_epi.astype(bool)

transB_mid = np.zeros((meshVtk.points.shape[0]))
transB_mid[(trans_distB.point_data['f_20']<=phi_epi) & (trans_distB.point_data['f_20']>=phi_endo)] = 1
