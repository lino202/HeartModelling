...
parser.add_argument('--useLayers', action='store_true')
args = parser.parse_args()

validKeys = ["endo", "endoBZ", "mid", "midBZ", "epi", "epiBZ", "myo", "scar", "bz", "patch", "unatt", "base_endo", "base_mid", "base_epi",
        ...
        for key in pointDataKeys:
            if pointData[key].ndim == 1 and "layers" in key:
                for pointKey in validKeys:
                    if (not "BZ" in pointKey) and (not "{}_nodes".format(pointKey) in nsets.keys()):
