...
from rbm.utils import getHugeNearest
...
endoRV = meshio.read(os.path.join(args.dataPath, 'mesh', 'rv_endo.obj')).points
...
idxs = getHugeNearest(endoRV, bmeshPoints, showMem=True)
endoRV = bmeshPoints[idxs,:]
base = np.delete(bmeshPoints, idxs,axis=0)

idxs = getHugeNearest(endoLV, base, showMem=True)
endoLV = base[idxs,:]
base = np.delete(base, idxs,axis=0)

idxs = getHugeNearest(epi, base, showMem=True)
epi = base[idxs,:]
base = np.delete(base, idxs,axis=0)

if "XV" in args.outName:
    bcArray0 = np.concatenate((endoLV,endoRV), axis=0)
    bcArray1 = epi
elif "RV" in args.outName:
    bcArray0 = endoRV
    bcArray1 = np.concatenate((endoLV,epi), axis=0)
elif "LV" in args.outName:
    bcArray0 = endoLV
