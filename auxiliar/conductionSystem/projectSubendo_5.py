...
import numpy as np 
import argparse
from lib.utils import checkEndBranchesOrder, getBranches, getProjectionDir, getProjectionMag, getLinearPath, getEdges, getProjectionMagLowMemory, reorderPurkMesh, saveVtkInpMesh1D
from lib.utils import checkRepeatedPoints, smoothProjection, updatePointsEdgesBranches, getProjectionMagLowMemory
import time
...
def main():
    ...
    parser.add_argument('--lowMemoryDivisions', type=int, help='Divisions to apply to CS purk branch so it takes less memory to compute the projection magnitude')
    ...
    for key in branches2Project:
        ...
        if args.projectRV or key!="rvb":
            ...
            if args.lowMemoryDivisions:
                mags = getProjectionMagLowMemory(tmpPoints, laplaciansMesh, subendoWindow, "lv" if "lv" in key else "rv", k=args.meanMag, divisions=args.lowMemoryDivisions)
            else:
    ...
    if args.project_intramyo: 
        ...
        for key in purkBranches.keys():
            ...
            for i in idxs2Mid:
                ...
                if args.lowMemoryDivisions:
                    mags = getProjectionMagLowMemory(endNodePoint, laplaciansMesh, intramyoWindow, "lv" if "lv" in key else "rv", k=args.meanMag, divisions=args.lowMemoryDivisions)
                else:
