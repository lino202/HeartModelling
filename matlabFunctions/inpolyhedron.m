...
if (isempty(options.facenormals));
    allFacetNormals=bsxfun(@times,allEdgeVecs(1,[2 3 1],:),...
        allEdgeVecs(2,[3 1 2],:))-...
        bsxfun(@times,allEdgeVecs(2,[2 3 1],:),allEdgeVecs(1,[3 1 2],:));
    allFacetNormals=bsxfun(@rdivide,allFacetNormals,...
        sqrt(sum(allFacetNormals.^2,2)));
else
...
% We use a Z-ray intersection so we don't even need to consider
% facets that are purely vertically oriented (have zero Z-component).
...
% If they provided X,Y,Z vectors of query points, our output is
% currently a 2D mask and must be reshaped to [LEN(Y) LEN(X) LEN(Z)].
IN=reshapeINfcn(IN);