...
% Adjust S0 such as S/S0 values later are decimal
S0 = 1000.;
...
% Get pixel physical dimensions
dirs = split(mask.metaData.space_directions);
...
% Get origin
orig = textscan(mask.metaData.space_origin,'(%f,%f,%f)');
oX = orig{1};
oY = orig{2};
oZ = orig{3};
...
points = zeros(nX*nY*nZ,3);              %Spacial
    ...
    for j=1:nY
        for i=1:nX
            if(mask.pixelData(i,j,k)~=0)
                ...
                points(id,:) = [(i-1)*sX+oX, (j-1)*sY+oY, (k-1)*sZ+oZ];
...
endo_nodes = find(part_flag==3);
mid_nodes = find(part_flag==2);
epi_nodes = find(part_flag==1);
scar_nodes = find(part_flag==4);
...
allparts_fd(endo_nodes) = 3;
allparts_fd(mid_nodes) = 2;
allparts_fd(epi_nodes) = 1;
allparts_fd(scar_nodes) = 4;
...
field_data = {{endo_fd,'endo_nodes'},...
              ...
              {fibs,'dti-fibers'}};
SavePointsToVtk(append(dataPath,'dti_voxel_fibers_scar.vtk'), points, field_data);
