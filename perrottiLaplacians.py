...
mesh = fenics.Mesh(os.path.join(rootPath, sample, "mesh.xml"))
fenics.info(mesh)
V = fenics.FunctionSpace(mesh, 'Lagrange', 1)

# Get the endo and epi surfaces
data = pd.read_csv(os.path.join(rootPath, sample, "endo_epi.csv"), usecols=['Points_0' , 'Points_1', 'Points_2', 'RegionId'])
data = data.to_numpy()
...
fenics.File(os.path.join(rootPath, sample,'{}.pvd'.format(outName))) << u

with open(os.path.join(rootPath, sample, '{}.txt'.format(outName)), "w") as file:
    file.write("# \n")
